...
export default class MessageForm extends ElementView {
    ...
    async connectedCallback () {
        ...
        this._audioState = "not_started";
        document.addEventListener("emojiSelected", this.handleEmojiSelection);
        
        console.log("Listen audioFileAvailable event here")
        //TODO: Fix binding event bug here
        document.addEventListener("audioStateChange", this.handleAudioStateChange.bind(this));
        
        this.render();
    }

    handleAudioStateChange(e){
      if(e.detail.state == "stop"){
        this._audioFile = e.detail.file; 
      }else if(e.detail.state == "clear"){
        this._audioFile = null;
      }
      this._audioState = e.detail.state;
    }
    ...
    async onFormSubmitted (ev) {
        ...
        if(this._audioState == "recording"){
          alert("You have not finished the current recording yet, please stop it before sending audio_file or clear it");
          return ;
        }
        ...
        if (
            ...
            !message_text.replace(/\s/g, '').length && !this._audioFile
        ) {
            return;
        }
        ...
        if(this._audioFile){
          await this.model.sendFiles([this._audioFile], message_text);
          this._audioFile = null;
          this.dispatchEvent(new CustomEvent("audioFileSended", {'bubbles': true,}));
          message = 1;
        } else{
          message = is_command ? null : await this.model.sendMessage({'body': message_text, spoiler_hint});
        }        
